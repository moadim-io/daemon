...
`restart`, `status`, `cleanup`, and `stop` each accept `--json` for a single-line, machine-readable
object on stdout. Paired with the exit codes above, a caller gets the full contract without parsing
prose:
...
| `moadim restart --json` | `{"old":N\|null,"new":M}` — `old` is the stopped server's PID (`null` when nothing was running), `new` is the freshly spawned PID | `0` (always restarts) |
