...
- Fold the `address` field into `moadim stop --json` and `cleanup --json` (`"address":"127.0.0.1:5784"`) so all three `--json` objects share the same `running`/`pid`/`address` envelope as `status --json`
- Add a CLI integration test that drives `moadim stop --json` against a real ephemeral listener and asserts the emitted `{"running":..,"pid":..}` object, covering the pid-before-shutdown read path end-to-end
