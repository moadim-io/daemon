...
pub enum Command {
    ...
    /// Register the daemon as an OS service (launchd on macOS, systemd --user on Linux) so it
    /// starts at login and survives reboots.
    Install,
    /// Deregister and remove the OS service installed by [`Command::Install`].
    Uninstall,
    /// Print usage help.
    Help,
...
pub fn print_help() {
    println!(
        "moadim — cron/MCP/REST server with a web control panel\n\
         ...
         \x20   install                register as an OS service (launchd/systemd) to run at login\n\
         \x20   uninstall              deregister and remove the OS service\n\
