...
#[test]
fn validate_machines_accepts_and_preserves_a_clean_list() {
    let input = vec!["alpha".to_string(), "beta".to_string()];
    assert_eq!(validate_machines(&input).expect("clean list"), input);
}

#[test]
fn validate_machines_trims_surrounding_whitespace() {
    let input = vec![" alpha ".to_string(), "\tbeta\n".to_string()];
    assert_eq!(
        validate_machines(&input).expect("trimmed"),
        vec!["alpha".to_string(), "beta".to_string()]
    );
}

#[test]
fn validate_machines_collapses_duplicates_after_trimming() {
    let input = vec![
        "host".to_string(),
        " host ".to_string(),
        "other".to_string(),
    ...
    assert_eq!(
        validate_machines(&input).expect("deduped"),
        vec!["host".to_string(), "other".to_string()]
    );
}

#[test]
fn validate_machines_rejects_empty_entry_with_indexed_error() {
    let input = vec!["alpha".to_string(), String::new()];
    let err = validate_machines(&input).expect_err("empty entry rejected");
    match err {
        AppError::BadRequest(msg) => assert!(msg.contains("machines[1]"), "msg: {msg}"),
        other => panic!("expected BadRequest, got {other:?}"),
    }
}

#[test]
fn validate_machines_rejects_whitespace_only_entry() {
    let input = vec!["   ".to_string()];
    let err = validate_machines(&input).expect_err("whitespace entry rejected");
    assert!(matches!(err, AppError::BadRequest(_)));
}

#[test]
fn validate_machines_accepts_empty_list_unchanged() {
    let input: Vec<String> = Vec::new();
    assert_eq!(validate_machines(&input).expect("empty list ok"), input);
}
