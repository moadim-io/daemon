...
fn reload_preserves_last_scheduled_trigger_at_sidecar() {
    // The reload goes through the same load path that reads the gitignored `scheduled.local.toml`
    // sidecar, so the scheduler-written `last_scheduled_trigger_at` survives a reload rather than
    // being clobbered.
    let dir = scratch_routines_dir();
    write_routine_to(&dir, &make_routine("sched"));
    // The launch command writes this sidecar at each scheduled firing; the daemon only reads it.
    std::fs::write(
        dir.join("sched").join("scheduled.local.toml"),
        "last_scheduled_trigger_at = 1717000000\n",
    )
    .unwrap();
    let store = new_store();
