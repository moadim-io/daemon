...
#[test]
fn parse_version_handles_valid_prerelease_and_invalid() {
    assert_eq!(parse_version("1.2.3"), Some((1, 2, 3)));
    // Pre-release / build metadata on the patch component is stripped before parsing.
    assert_eq!(parse_version("0.12.0-rc.1"), Some((0, 12, 0)));
    assert_eq!(parse_version(" 2.4.6+build9 "), Some((2, 4, 6)));
    // Fewer than three components, or non-numeric components, are unparsable.
    assert_eq!(parse_version("1.2"), None);
    assert_eq!(parse_version("a.b.c"), None);
    // The crate's own version is always a parsable semver core.
    assert!(parse_version(env!("CARGO_PKG_VERSION")).is_some());
}

#[test]
fn script_stamped_version_reads_present_absent_and_missing() {
    let base = std::env::temp_dir().join(format!("moadim-stamp-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&base).unwrap();

    let stamped = base.join("stamped.sh");
    std::fs::write(
        &stamped,
        format!("#!/bin/sh\n{VERSION_STAMP_PREFIX}3.4.5\necho hi\n"),
    )
    .unwrap();
    assert_eq!(script_stamped_version(&stamped), Some((3, 4, 5)));

    // A script without the stamp line (e.g. written before the stamp existed) yields None.
    let unstamped = base.join("unstamped.sh");
    std::fs::write(&unstamped, "#!/bin/sh\necho hi\n").unwrap();
    assert_eq!(script_stamped_version(&unstamped), None);

    // A missing file is unreadable → None.
    assert_eq!(script_stamped_version(&base.join("nope.sh")), None);

    let _ = std::fs::remove_dir_all(&base);
}

#[test]
fn write_routine_script_stamps_current_version() {
    let title = "Stamp Write Sync Routine";
    let slug = slugify(title);
    let _ = std::fs::remove_dir_all(crate::paths::routine_dir(&slug));
    let routine = make_routine("stamp-write", title, "claude");
    let agent = AgentCommand {
        command: "claude".to_string(),
        args: vec![],
        setup: None,
    ...
    let script = std::fs::read_to_string(crate::paths::routine_script_path(&slug)).unwrap();
    // Second line stamps the generating daemon version.
    assert_eq!(
        script.lines().nth(1).unwrap(),
        format!("{VERSION_STAMP_PREFIX}{}", env!("CARGO_PKG_VERSION"))
    );
    // The regenerated run.sh still carries the routine-origin disclosure (regression guard #167).
    assert!(script.contains("Routine origin disclosure"));
    let _ = std::fs::remove_dir_all(crate::paths::routine_dir(&slug));
}

#[test]
fn write_routine_script_refuses_to_overwrite_newer_stamped_script() {
    // Downgrade guard (#167): a run.sh stamped by a newer daemon must not be clobbered by an older
    // binary, which would silently strip the disclosure block.
    let title = "Newer Stamp Sync Routine";
    let slug = slugify(title);
    let path = crate::paths::routine_script_path(&slug);
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    let newer = format!("#!/bin/sh\n{VERSION_STAMP_PREFIX}999.0.0\n# newer-daemon body\n");
    std::fs::write(&path, &newer).unwrap();

    let routine = make_routine("newer-stamp", title, "claude");
    let agent = AgentCommand {
        command: "claude".to_string(),
        args: vec![],
        setup: None,
    };
    // The line still resolves (points at the preserved script), but the file is left untouched.
    let line = format_routine_line(&routine, &agent).unwrap();
    assert!(line.ends_with("# moadim-routine:newer-stamp"));
...
#[test]
fn write_routine_script_overwrites_older_stamped_script() {
    // The normal upgrade direction: an older-stamped run.sh is regenerated by the current binary.
    let title = "Older Stamp Sync Routine";
    let slug = slugify(title);
    let path = crate::paths::routine_script_path(&slug);
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    std::fs::write(
        &path,
        format!("#!/bin/sh\n{VERSION_STAMP_PREFIX}0.0.1\n# stale body\n"),
    )
    .unwrap();

    let routine = make_routine("older-stamp", title, "claude");
    let agent = AgentCommand {
        command: "claude".to_string(),
        args: vec![],
        setup: None,
    ...
    assert!(!script.contains("stale body"));
