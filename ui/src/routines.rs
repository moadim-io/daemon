...
pub enum RAction {
    ...
    /// Apply a selection click interpreted per `kind`.
        ...
        match action {
            ...
            RAction::SelectRoutine { id, kind } => match kind {
                ...
                SelectKind::Range => {
                    ...
                    let pos = |target: &str| s.routines.iter().position(|x| x.id == target);
...
pub fn routines_page(props: &RoutinesPageProps) -> Html {
    ...
    // over the filtered+sorted rows so select-all never reaches hidden routines.
    let on_select_all = {
        let state = state.clone();
        let now = now.clone();
    ...
    // Bulk enable/disable: PATCH each selected routine, emit one summary toast.
...
pub struct TableProps {
    ...
    /// Currently selected routine ids.
    pub selected: HashSet<String>,
